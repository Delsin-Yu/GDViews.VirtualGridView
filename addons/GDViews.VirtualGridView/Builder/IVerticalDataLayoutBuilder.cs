...
/// Use the <see cref="AppendDataSet"/> method to build up the vertical datasets,
...
public interface IVerticalDataLayoutBuilder<TDataType> : IFinishingBuilderAccess<TDataType>
{
    ...
    /// <param name="repeatCount">The xs this dataset takes, increase this value is equatable for calling this API multiple times.</param>
    ...
    ///             ^^ +New ^^
    /// </code></remarks>
    IVerticalDataLayoutBuilder<TDataType> AppendDataSet(IDynamicGridViewer<TDataType> dataSet, int repeatCount = 1);
}